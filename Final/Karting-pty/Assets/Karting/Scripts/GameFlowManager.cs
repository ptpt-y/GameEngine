...
public class GameFlowManager : MonoBehaviour
{
    ...
    void EndGame(bool win)
    {
        ...
        if (win)
        {
            ...
            // store time score
            string curGame = SceneManager.GetActiveScene().name+"#";
            //float usedTime = m_TimeManager.TotalTime - m_TimeManager.TimeRemaining;
            float usedTime = 90f - m_TimeManager.TimeRemaining;
            curGame += usedTime.ToString();
            curGame += "#";
            curGame += m_ScoringSystem.GetStaticScoreNum().ToString();
            curGame += "#";
            curGame += System.DateTime.Now.ToString();
            Debug.Log(curGame);
            PlayerPrefs.SetFloat("curScore", 60 - m_TimeManager.TimeRemaining);
            PlayerPrefs.SetString("curGame", curGame);
            m_ScoringSystem.PassScore(m_ScoringSystem.GetStaticScoreNum(), usedTime);
            m_ScoringSystem.UpdateScoreBoard();
        }
        else
        {
            m_ScoringSystem.PassScore(m_ScoringSystem.GetStaticScoreNum(), 90f);
